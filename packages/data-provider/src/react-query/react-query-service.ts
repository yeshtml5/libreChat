...
> => {
  ...
  return useMutation({
    ...
    onSuccess: (data, variables) => {
      queryClient.invalidateQueries({ queryKey: [QueryKeys.name, variables.name] });
    },
  });
};
...
export const useRevokeUserKeyMutation = (name: string): UseMutationResult<unknown> => {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: [QueryKeys.name, name] });
      if (s.isAssistantsEndpoint(name)) {
        queryClient.invalidateQueries({ queryKey: [QueryKeys.assistants, name, defaultOrderQuery] });
        queryClient.invalidateQueries({ queryKey: [QueryKeys.assistantDocs] });
...
export const useRevokeAllUserKeysMutation = (): UseMutationResult<unknown> => {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: [QueryKeys.name] });
...
> => {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: [QueryKeys.tokenCount] });
    },
  });
};
...
export const useAvailablePluginsQuery = <TData = s.TPlugin[]>(
  config?: UseQueryOptions<s.TPlugin[], Error, TData>,
): QueryObserverResult<TData, Error> => {
  return useQuery<s.TPlugin[], Error, TData>({
...
> => {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      queryClient.refetchQueries({ queryKey: [QueryKeys.tools] });
    },
  });
};
...
export const useUpdateFeedbackMutation = (
  ...
  return useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: [QueryKeys.messages, messageId] });
    },
  });
};
